    ...
    return data.couleurs.map((couleur, index) => {
      if (typeof couleur === "string") {
        ...
        return {
          nom: couleur,
          hex: "#1A1A1A",
          images: imageUrl ? [{ imageUrl, imagePublicId: "" }] : [],
          newFiles: []
        };
      }

      if (Array.isArray(couleur.images)) {
        return {
          nom: couleur.nom || "",
      ...
      const imageUrl = couleur.imageUrl || "";
      const imagePublicId = couleur.imagePublicId || "";
      return {
        nom: couleur.nom || "",
        hex: couleur.hex || "#1A1A1A",
        images: imageUrl ? [{ imageUrl, imagePublicId }] : [],
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between border-b border-black/[0.05] pb-8 animate-fade-up">
        <div>
          ...
          <p className="mt-1 text-xs text-[#7a7368]">
            Mettez à jour les caractéristiques, le stock et les déclinaisons couleurs de l'article.
          </p>
        </div>
        <Link
          to="/admin/produits"
